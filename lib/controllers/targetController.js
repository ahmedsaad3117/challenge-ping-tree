...
function createTarget (...params) {
  ...
  if (opts.body.id && opts.body.url && opts.body.value) {
    redis.HSET('targets', opts.body.id, JSON.stringify(opts.body),
      function (err, result) {
        if (err) onError(err)
    ...
    sendJson(req, res, { message: 'Please provide the suitable data' })
  }
}
...
function updateTargetById (...params) {
  ...
  redis.HGET('targets', opts.params.id, function (err, result) {
    ...
    if (result) {
      ...
      redis.HSET('targets', opts.params.id, JSON.stringify(newDoc),
        function (err, result) {
          if (err) onError(err)
          if (!result) {
  ...
  redis.HGETALL('targets', function (err, result) {
    ...
    if (result) {
      ...
      var allowedTargets = targets.filter((target) => {
        ...
        return targetGeoStateAllow.includes(currentVisitorGeoState) &&
         targetTimeAllow.includes(currentVisitorHour.toString())
      })

      allowedTargets = allowedTargets.sort((a, b) =>
        parseFloat(b.value) - parseFloat(a.value)).sort((a, b) =>
      ...
      redis.GET(`targetReq:${decisionTarget.id}:${formattedDate}`,
        function (err, remainingCounter) {
          if (err) onError(err)
          ...
          setRequestsRemaining(decisionTarget.id, formattedDate,
            remainingCounter - 1, onError, function () {
              return sendJson(req, res, {
                url: decisionTarget.url,
                decision: decisionTarget.value
              })
            })
        })
    } else {
